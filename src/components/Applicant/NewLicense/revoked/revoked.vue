    ...
    <main-content>
        <nav class="bg-gray-100 px-5 py-3 rounded-md w-full">
            <ol class="list-reset flex">
                ...
                <li><span class="text-gray-500 mx-2">/</span></li>
                <li class="text-gray-500">Revoked</li>
            </ol>
        </nav>
        ...
        <div class="container my-12 mx-auto px-4 md:px-12" v-if="noData == false">
            <div class="flex flex-wrap sm:-mx-1 lg:-mx-4">
                <div v-for="revokedLicense in revokedLicenses" :key="revokedLicense.id" class="
              bg-white
              my-1
              px-1
              md:w-1/4
            ...
            ">
                        ...
                        <div class="border-b-2 text-main-400">
                            <div class="grid grid-rows-2 p-2 mb-2 border-b-2">
                                ...
                                <ul class="text-black text-sm">
                                    <li v-for="(education, index) in revokedLicense.educations" :key="education.id"
                                        ...
                                        <span class="text-black text-sm">
                                            ...
                                            <span v-if="index != revokedLicense.educations.length - 1">
                                                ,
                                            </span>
                                        </span>
                                    </li>
                  ...
                  ">
                                ...
                                <p class="text-black text-sm">
                                    {{
                                    revokedLicense.certifiedDate
                                    ? revokedLicense.certifiedDate.slice(0, 10)
                                    : "Waiting for review"
                                    }}
                                </p>
                            </div>
                  ...
                  ">
                                ...
                                <p class="text-black text-sm">
                                    {{
                                    revokedLicense.licenseExpirationDate
                                    ? revokedLicense.licenseExpirationDate.slice(0, 10)
                                    : "Waiting for review"
                                    }}
                                </p>
                            </div>
                  ...
                  " href="#">
                            ...
                            revokedLicense.createdAt.slice(0, 10)
                            }}</span>
                        </footer>
                    ...
                    duration-150
                    ease-in-out
                  " @click="openRevokedDetail(revokedLicense.id)" data-bs-toggle="modal"
                                data-bs-target="#revokedDetail">
                                View Detail
                            </button>
                        </div>
                    </div>
        ...
        ">
            ...
            <h2 class="text-main-400 border-b-2 text-xl p-2">
                There are no revoked applications currently.
            </h2>
        </div>
        <revoked-detail :modalDataId="modalDataId"></revoked-detail>
    </main-content>
</template>
...
import revokedDetail from "./revokedDetail.vue";

export default {
    components: { MainContent, revokedDetail },
    setup() {
        let isLoading = ref(false);
        let noData = ref(false);
        let userInfo = ref({});
        let store = useStore();
        let revokedLicenses = ref([]);
        ...
        onMounted(() => {
            ...
            store.dispatch("newlicense/getNewLicenseByUser", userId).then((res) => {
                ...
                if (results.length > 0) {
                    revokedLicenses.value = results.filter((revokedLicense) => {
                        return revokedLicense.applicationStatus.code === "RVK";
                    });

                    if (revokedLicenses.value.length === 0) {
                        noData.value = true;
                    }

  ...
  <main-content>
    ...
    <div class="container my-12 mx-auto px-4 md:px-12" v-if="noData == false">
      <div class="flex flex-wrap sm:-mx-1 lg:-mx-4">
        ...
        <div
          v-for="license in undersupervisionLicenses"
          :key="license.id"
          class="
            bg-white
            my-1
          ...
          <div>
            ...
            <div class="flex justify-center">
              <button
                ...
                data-bs-target="#undersupervisionDetail"
              >
                View Detail
              </button>
            </div>
...
import undersupervisionDetail from "./underSupervisionDetail.vue";

export default {
  components: { MainContent, undersupervisionDetail },
  setup() {
    let store = useStore();
    let undersupervisionLicenses = ref([]);
    let userInfo = ref({});
    ...
    onMounted(() => {
      ...
      store.dispatch("newlicense/getNewLicenseByUser", userId).then((res) => {
        ...
        if (results.length > 0) {
          undersupervisionLicenses.value = results.filter(
            (uSupervisionLicense) => {
              return uSupervisionLicense.applicationStatus.code === "USUP";
            }
          );

          if (undersupervisionLicenses.value.length === 0) {
            noData.value = true;
          }

...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      console.log(
        "check",
        generalInfo.value.multipleDepartment[0].educationalLevel.id
      );
      ...
      generalInfo.value
        ...
        : "";
      let documentsStored = store.getters["renewal/getDocumentSpec"];
      documents.value = documentsStored.filter((element) => {
        if (element.isCommonDocument) {
          return element;
        }
      });
      documents.value = documents.value.filter((element) => {
        if (
          element.applicantType?.id ==
          generalInfo.value.applicantTypeSelected.id
        ) {
          return element;
        }
      });
      departmentDocuments.value = documentsStored.filter((element) => {
        if (
      ...
      departmentDocuments.value = departmentDocuments.value.filter(
        (element) => {
          let temp = false;
          generalInfo.value.multipleDepartment.forEach((val) => {
            if (val.educationalLevel?.id == element.educationalLevel?.id) {
              temp = true;
            }
          });
          if (temp) {
      ...
      console.log(departmentDocuments.value);
    });
    return {
      documents,
      files,

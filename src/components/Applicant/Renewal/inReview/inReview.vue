  ...
  <main-content>
    ...
    <div class="container my-12 mx-auto px-4 md:px-12" v-if="noData == false">
      <div class="flex flex-wrap sm:-mx-1 lg:-mx-4">
        ...
        <div v-for="renewal in inReviewRenewals" :key="renewal.id" class="
            bg-white
            my-1
            px-1
            md:w-1/4
    ...
    " href="#">
            ...
            <div class="flex justify-center">
              <button class="
                  ...
                  duration-150
                  ease-in-out
                " @click="openInReviewDetail(renewal.id)" data-bs-toggle="modal" data-bs-target="#inReviewDetail">
                View Detail
              </button>
            </div>
          </div>
      ...
      store.dispatch("renewal/getRenewalsByUser", userId).then((res) => {
        ...
        if (results.length > 0) {
          inReviewRenewals.value = results.filter(inReviewRenewal => {
            return (
              inReviewRenewal.applicationStatus.code === "IRV"
            );
          });

          if (inReviewRenewals.value.length === 0) {
            noData.value = true;
          }

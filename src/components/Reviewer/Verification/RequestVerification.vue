      ...
      >
        <div class="flex">
            ...
            <ul class="list-reset">
              <li class="mr-3">
                <input
                  ...
                  v-model="searchInput"
                  @input="searchedUserInput"
                /><br />
              </li>
              <div
                class="relative pin-t pin-l custom-lookup overflow-y-auto"
                v-if="showCustomLookup"
              >
          ...
          <div class="flex flex-col mb-medium w-1/2">
            ...
            <div>
              <select class="max-w-3xl" v-model="region">
                <option
        ...
        <button class="text-primary mt-5 float-right" type="submit">
          Submit
        </button>
      </form>
    </div>
  ...
  setup({ emit }) {
    ...
    const searchedUserInput = () => {
      ...
      filteredUsers.value = users.value.filter((x) => {
        return (
          x.name.toLowerCase().includes(searchInput.value) ||
          x.fatherName.toLowerCase().includes(searchInput.value) ||
          x.grandFatherName.toLowerCase().includes(searchInput.value)
        );
      });
    };

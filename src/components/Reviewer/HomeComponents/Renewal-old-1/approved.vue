...
import NewLicenseMainContent from "../../ApplicationTypes/Renewal/MainComponents/approved.vue";
...
export default {
  ...
  setup() {
    ...
    const approvedByOthers = () => {
      loading.value = true;
      applicationStatus(store, "CONF").then(res => {
        let statusId = res;
      ...
      applicationStatus(store, "APP").then(res => {
        ...
        store
          ...
          .then(() => {
            ...
            allInfo.value.assignApplication =
              store.getters["reviewerRenewal/getRenewalApprovedSearched"];

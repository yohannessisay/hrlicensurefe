      ...
      <ol class="list-reset flex">
        <li>
          <router-link to="/admin/declined"
            ><span class="text-primary-600 text-base mt-2">Home</span></router-link
          >
        </li>
...
import editModalOthers from "./underSupervisionModalOthers.vue";
export default {
  name: "home",
  components: {
    ReviewerSideNav,
  ...
  setup() {
    ...
    const supervisedByYou = (apiParameters) => {
      let statId = statuses
    ...
    const supervisedByOthers = (apiParameters) => {
      let statId = statuses
        ? statuses.filter((stat) => stat.code == "USUP")[0].id
        : "";
        tableData = [];

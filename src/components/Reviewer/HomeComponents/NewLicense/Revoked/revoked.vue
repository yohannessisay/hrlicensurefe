...
import editModal from "./revokedModal.vue";
import editModalOthers from "./revokedModalOthers.vue";
...
export default {
  ...
  setup() {
    ...
    const revoked = () => {
      let revokeStatus = store.getters["applicationStatuses/getAppStatuses"]
     
      revokeStatus =
        revokeStatus && revokeStatus.length > 0
          ? JSON.parse(JSON.stringify(revokeStatus.filter((stat) => stat.code == "RVK")))
          : "";
          console.log(store.state.appStatuses)
      store
        .dispatch("reviewerNewLicense/getNewLicenseRevoked", revokeStatus[0].id)
        .then((res) => {

      ...
      <div class="container px-4 sm:px-8">
        <div class="relative py-4">
          <p class="absolute left-0 text-2xl font-semibold leading-tight">
            View and manage document types found throughout the system
          </p>
        ...
        <div class="w-full mt-8 rounded-xl">
          <vue-table-lite
          :is-static-mode="true"
            :is-loading="documentTypeTable.isLoading"
            :columns="documentTypeTable.columns"
            :rows="documentTypeTable.rows"
            :total="documentTypeTable.totalRecordCount"
            :sortable="documentTypeTable.sortable"
...
export default {
  ...
  setup() {
    ...
    let documentTypeTable = ref({ isLoading: true });
    let documentTypeTableData = [];
    let showAddButton = ref(false);
    let editModalData = ref({ finalStatus: false, Name: "" });
    const fetchDepartments = () => {
      store.dispatch("lookups/getDocumentType").then((res) => {
        res.data.data.forEach((element) => {
          documentTypeTableData.push({
        ...
        documentTypeTable.value = {
          ...
          rows: documentTypeTableData,
          totalRecordCount: documentTypeTableData.length,
          sortable: {
            order: "id",
            sort: "asc",
          },

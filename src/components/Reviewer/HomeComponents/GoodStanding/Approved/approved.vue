...
export default {
  ...
  setup() {
    ...
    const approvedByYou = () => {
      applicationStatus(store, "APP").then((resApp) => {
        applicationStatus(store, "AP").then((resAp) => {
          let statusId = resApp;
          let approvedStatus = resAp;
          let adminStatus = [adminId, statusId, approvedStatus];
          store
            .dispatch(
              "reviewerGoodStanding/getGoodStandingLicensed",
              adminStatus
            )
            .then((res) => {
              allInfo.value.assignApplication =
